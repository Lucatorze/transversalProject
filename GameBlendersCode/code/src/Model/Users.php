...
class Users{
    ...
    public static function getNbOffer($pdo){
        $stmt = $pdo->prepare("SELECT COUNT(id) as countid FROM offers WHERE userId = :userId");
    ...
    public static function getOtherNbOffer($pdo, $id){
        $stmt = $pdo->prepare("SELECT COUNT(id) as countid FROM offers WHERE userId = :userId");
